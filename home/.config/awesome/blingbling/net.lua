...
local function show_ippopup_infos(n_graph)
  ...
  if data[n_graph][interface.."_state"] == "up" then
    if data[n_graph][interface.."_carrier"] == "1" then --get local ip configuration
      ...
      local ext_ip
      if data[n_graph].ext_ip_timer == nil or data[n_graph].ext_ip_timer + 2000 < os.time() then
        ext_ip = awful.util.pread("curl --silent http://automation.whatismyip.com/n09230945.asp")
        data[n_graph].ext_ip = ext_ip
        data[n_graph].ext_ip_timer=os.time()
      else
        ext_ip = data[n_graph].ext_ip
      end

      local separator ="\n|\n"
      text="Local Ip:\t"..ip_addr..separator.."Gateway:\t".. gateway..separator .."External Ip:\t"..ext_ip
    else
